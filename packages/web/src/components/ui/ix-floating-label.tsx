...
export function FloatingLabelInput({
  ...
  const attach = useCallback(
    (node: HTMLInputElement | null) => {
      ref.current = node;
      if (inputRef instanceof Function) inputRef(node);
  ...
  return (
    <div className={`w-full ${className}`}>
      <div className="relative pt-[20px]">
        <div
          className={`relative h-10 rounded-lg border-2 transition-[background-color,border-color,box-shadow] duration-150 ${
            invalid
        ...
        >
          ...
          onChange={(event) => {
            ...
            className="absolute inset-0 h-full w-full rounded-md bg-transparent px-3 py-0 text-[13px] leading-[20px] text-foreground outline-none focus-visible:outline-none disabled:cursor-not-allowed"
          />
        </div>
        ...
        >
          {label}
          {required ? (
            <span aria-hidden className="ml-0.5 text-muted-foreground/70">
              *
            </span>
          ) : null}
        </motion.label>
      ...
      <div className="mt-1.5 flex h-[16px] items-start gap-3">
        ...
        {maxLength !== undefined ? (
          <span
            aria-hidden
            className="grid shrink-0 justify-items-end font-mono text-[10.5px] leading-[16px] tabular-nums text-muted-foreground/70"
          >
