...
export function Accordion({
  ...
  return (
    <div
      className={`divide-y divide-border overflow-hidden rounded-lg border border-border bg-card text-card-foreground shadow-sm ${className}`}
    >
      {items.map((item) => (
        <AccordionRow
          key={item.id}
...
function AccordionRow({
  ...
  useEffect(() => {
    // SAFETY: the ref holds the panel element this component renders.
    // `Inertable` only adds the `inert` boolean that browsers expose and this
    // project's DOM lib does not declare.
  ...
  return (
    <div>
      <div role="heading" aria-level={headingLevel}>
        <button
          {...header}
          className="flex w-full items-center gap-3 px-3.5 py-3 text-left outline-none transition-colors duration-150 hover:bg-muted focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring"
        >
          <span
