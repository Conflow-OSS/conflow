...
export function useSliderDetents({
  ...
  const list = useMemo<SliderDetent[]>(
    () => detents.map((d) => (isNumber(d) ? { value: d } : d)),
    [detents],
  );
  ...
  return (
    <div className={`w-full select-none ${className}`}>
      ...
      <div
        ref={trackRef}
        {...trackProps}
        className={`relative h-9 w-full rounded-md outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0 ${
          disabled
      ...
      >
        <div className="pointer-events-none absolute inset-x-0 top-[9px] h-[10px] overflow-hidden rounded-sm bg-muted">
          <div
        ...
        >
          ...
          >
            <motion.div
              className="absolute top-[4px] h-[20px] w-[18px] rounded-md border-2 border-background bg-primary"
