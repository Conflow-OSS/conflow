      ...
      <AnimatePresence initial={false}>
        {activity ? (
          <motion.div
            ...
            className="pointer-events-auto relative overflow-hidden rounded-lg border border-border bg-card text-card-foreground shadow-lg"
          >
            <motion.div
              ref={compactRef}
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {activity.action ? (
                  <button
                    ...
                    className="inline-flex h-[24px] shrink-0 select-none items-center whitespace-nowrap rounded-md border border-border bg-card px-2 text-[11px] font-medium text-foreground shadow-sm outline-none transition-[background-color,border-color,box-shadow] duration-150 hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0 active:translate-y-px"
                  >
                ...
                {onDismiss && phase !== "running" ? (
                  <button
                    ...
                    onClick={onDismiss}
                    className="grid size-[22px] shrink-0 place-items-center rounded-md text-muted-foreground/70 transition-colors duration-150 hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0"
                  >
              ...
              {percent !== null && phase !== "error" ? (
                <div className="mt-2.5 flex items-center gap-2 pl-[26px]">
                  <div className="min-w-0 flex-1 rounded-sm bg-muted p-[2px]">
...
function PhaseGlyph({ phase, reduced }: { phase: ActivityPhase; reduced: boolean }) {
  return (
    <span className="grid size-[18px] shrink-0 place-items-center">
      ...
      >
        {reduced ? (
          <svg width="13" height="13" viewBox="0 0 12 12" aria-hidden>
            <circle cx="6" cy="6" r="4.4" stroke="currentColor" strokeWidth="1.6" fill="none" className="text-muted-foreground/70" opacity="0.4" />
          </svg>
        ) : (
          <motion.svg
            width="13"
      ...
      <motion.span
        className="col-start-1 row-start-1 flex text-[var(--success,oklch(0.63_0.15_155))]"
        initial={false}
        animate={{
