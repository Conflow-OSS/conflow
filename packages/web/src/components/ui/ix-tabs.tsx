...
// Tabs: keyboard-navigable tab strip whose selected plateau slides between triggers. Restyled onto the
...
export function Tabs({
  ...
  return (
    <div
      className={`w-full overflow-hidden rounded-lg border border-border bg-card shadow-sm ${className}`}
    >
      <div
        {...tabs.tabListProps}
        ref={rowRef}
        ...
        {items.map((item, index) => {
          ...
          return (
            <button
              ...
              className={`relative flex h-8 shrink-0 items-center justify-center rounded-t-md px-3.5 text-[12.5px] outline-none transition-colors duration-150 after:pointer-events-none after:absolute after:inset-0 after:rounded-t-md after:content-[''] focus-visible:after:ring-2 focus-visible:after:ring-inset focus-visible:after:ring-ring ${
                item.disabled
      ...
      {renderPanel ? (
        <motion.div
          ...
          className={`rounded-lg text-[13.5px] leading-relaxed text-card-foreground outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring ${panelClassName}`}
        >
